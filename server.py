...
def midi_to_note(midi_num):
    return NOTE_NAMES[midi_num % 12], (midi_num // 12) - 1
...
def detect_key(note_counts):
    ...
    for i in range(12):
        corr_major = float(np.corrcoef(note_counts, major_profile[i:] + major_profile[:i])[0,1])
        if corr_major > best_corr:
            best_corr, best_key, best_mode = corr_major, NOTE_NAMES[i], "major"
        corr_minor = float(np.corrcoef(note_counts, minor_profile[i:] + minor_profile[:i])[0,1])
        if corr_minor > best_corr:
            best_corr, best_key, best_mode = corr_minor, NOTE_NAMES[i], "minor"
...
def notes_to_chords(active_notes):
    ...
    root = NOTE_NAMES[indices[0]]
    intervals = [(i - indices[0]) % 12 for i in indices[1:]]
...
def transcribe():
    ...
    try:
        ...
        t = 0.0
        while t < max_time:
            active = [n["midi"] for n in notes_result if n["start"] <= t + 0.5 and n["end"] >= t]
        ...
        print(f"결과 전송! 코드 수: {len(chords_result)}, Key: {key_display}", flush=True)
